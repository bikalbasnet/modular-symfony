...
class Voucher
{
    ...
    private bool $new_customers_only;
    ...
    #[ORM\Column(type: 'boolean', length: 4, nullable: true, options: ["default" => false])]
    private bool $active_on_campaign;

    #[ORM\Column(type: 'string', length: 100, nullable: true)]
    private string $usage_limit_restriction;
    ...
    #[ORM\OneToMany(mappedBy: 'voucher_id', targetEntity: VoucherConstraint::class)]
    private $voucherConstraints;

    public function __construct()
    {
        $this->voucherConstraints = new ArrayCollection();
    }
    ...
    /**
     * @return Collection|VoucherConstraint[]
     */
    public function getVoucherConstraints(): Collection
    {
        return $this->voucherConstraints;
    }

    public function addVoucherConstraint(VoucherConstraint $voucherConstraint): self
    {
        if (!$this->voucherConstraints->contains($voucherConstraint)) {
            $this->voucherConstraints[] = $voucherConstraint;
            $voucherConstraint->setVoucherId($this);
        }

        return $this;
    }

    public function removeVoucherConstraint(VoucherConstraint $voucherConstraint): self
    {
        if ($this->voucherConstraints->removeElement($voucherConstraint)) {
            // set the owning side to null (unless already changed)
            if ($voucherConstraint->getVoucherId() === $this) {
                $voucherConstraint->setVoucherId(null);
            }
        }
